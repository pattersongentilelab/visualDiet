...
for i = 1:length(participants)
    ...
    for d = 1:length(Day)
        ...
        Min500_B10mEDI(d,1) = length(vd_cleaned.MELANOPICEDI(vd_cleaned.LIGHT>500 & vd_cleaned.day==Day(d)));
...
B10_250m = NaN*ones(length(participants),1); B10_500m = NaN*ones(length(participants),1);
...
for i = 1:length(participants)
    ...
    B10_500m(i,:) = nanmedian(T.mEDI_min500(T.record_id==participants(i) & T.GoodDay==1));
...
M.Light = Light; M.mEDI = mEDI; M.B10_500m = B10_500m; M.B10_250m = B10_250m;
...
save([data_path '/pilotVDgrant'],'M','T','SPL','subject_data')
